...
새롭게 설정한 단위계는 지구 반지름 기준 로그 스케일로 압축된 거리 + 독립적으로 확대된 오브젝트 조합을 기반으로 함.
이 조합을 통해서 각 오브젝트의 공전 궤도를 명확하게 보여주는 학습자료를 만들고자 하는 목적으로 단위계를 설정함.
각각의 오브젝트의 공전궤도와 그 배치에 집중할 수 있는 단위계를 제작함.

따라서 만약에 하나의 오브젝트를 더 추가한다면, 거의 90도에 가까운 각도로 극궤도를 도는 위성을 하나 더 추가하자 한다.
```
거리 로그스케일 압축: f(r) = log(1+r/지구r) (자연로그)
```
**변경되는 값**
...
인공위성 궤도가 너무 낮아 인공위성이 지구와 겹치는 문제를 해결하기 위해 인공위성 궤도의 반지름을 1.5배 확장하는 시각화 보정 배율이 추가로 적용함.

**공전속도 배율**

- 실제 공전 속도 기준: 달 1.022 km/s, 인공위성 7.64 km/s
- 시각화 과정에서 움직임을 확인하기 어렵다는 문제를 해결하기 위해 두 오브젝트 모두 50배의 속도 보정을 적용.
- 단, 실제 물리 시뮬레이션에서는 로그 단위 축척을 적용하는 과정에서 각속도가 달라지고, 거리와 속도의 값이 달라지므로, 이는 단순히 시각화용 보정값임을 명시함.

- 달: 1.022 km/s × 50 = 51.1 km/s
- 인공위성: 7.64 km/s × 50 = 382 km/s

**전체 좌표계: 1 -> 3**

전체 좌표계의 시각적 크기를 1에서 3으로 확대하여 오브젝트와 공전 궤도를 더욱 명확하게 관찰할 수 있도록 함.
위의 보정이 모두 적용된 천체는 "실제 비율" 이 아닌, 달과 인공위성의 공전궤도를 보여주기 위한 시각적 교육자료임.
...
- 오브젝트 간의 거리는 지구 반지름을 기준으로 삼는 로그 스케일로, 오브젝트의 크기는 일정 비율 과장하는 복합 묘사법이다. 왜냐하면 이 페이지를 만드는 이유는 행성과 인공위성의 공전 궤도의 형태를 보여주고자 하는 목적이지, 그 사이의 실질적인 거리를 측정하기 위한 이유가 아니기 때문임.
또한 동일한 이유로 공전속도 또한 실제에 속도를 기반으로 배율을 적용할 예정.

제안한 방법의 장점과 잃는 것을 함께 쓰세요.
- 위성과 달이라는 두 가지 중요한 오브젝트에 초점을 맞춰 구도를 관측할 수 있음. 이 둘이 어떤 궤도로 공전하는지를 한눈에 알아보기 좋지만, Trade-off로 그 둘이 천문학적인 관점에서 얼마나 떨어져 있는지는 알아볼 수 없음.
따라서 이 축척법은 광활한 우주를 실제 축척으로 보여줘야 하는 스페이스 오페라 장르의 영화나 다큐멘터리에는 적합하지 않은 축척법임.
